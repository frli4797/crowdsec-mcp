...
- `cscli` for alert reads and explicit single-IP ban/unban write actions
...
| `WRITE_AUDIT_LOG_PATH` | JSON Lines audit trail for prepared and executed write attempts, defaults to `crowdsec-write-audit.jsonl` |
| `LOG_LEVEL` | Python log level, defaults to `INFO`; use `DEBUG` to echo MCP client tool activity |

Write tools validate a single IP, prepare a plausible `cscli` command for operator review, and append the write attempt to the JSON Lines audit log. `ban_ip` and `unban_ip` execute with `cscli` only when `execute=true`; `allow_ip` remains prepare-only.
...
Docker is the first supported deployment model. Run it on the same Docker network as CrowdSec, or on a host that can reach CrowdSec LAPI and invoke a narrowly scoped `cscli` path for executed ban/unban actions.
...
The second call returns a prepared, audited `potential_cscli_command`. Add `"execute": true` only after reviewing the command and deciding the single-IP ban should be applied.

## Safety Model

- Read-only CrowdSec tools are the default workflow.
- Write tools only operate on a single IP.
- `ban_ip` and `unban_ip` execute `cscli` only when `execute=true`.
- `allow_ip` remains prepare-only.
...
- Prepared and executed write attempts are appended to the JSON Lines audit log.
